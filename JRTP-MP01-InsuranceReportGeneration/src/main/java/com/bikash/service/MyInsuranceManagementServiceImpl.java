...
public class MyInsuranceManagementServiceImpl implements IInsuuranceManagementService {
	...
	public List<Insurance_Citizen> getAllCitizen(RequestInsuranceMapping reqMap) {
		...
		if(reqMap.getPlanName()!=null && reqMap.getPlanName()!="")
		{
			citizen.setPlanName(reqMap.getPlanName());
		}
		if(reqMap.getPlanStatus()!=null && reqMap.getPlanStatus()!="")
		{
			citizen.setPlanStatus(reqMap.getPlanStatus());
		}
		if(reqMap.getGender()!=null && reqMap.getGender()!="")
		{
			citizen.setGender(reqMap.getGender());
		}
		if(reqMap.getPlanStartDate()!=null)
		{
			citizen.setPlanStartDate(reqMap.getPlanStartDate());
		}
		if(reqMap.getPlanEndDate()!=null)
		{
			citizen.setPlanEndDate(reqMap.getPlanEndDate());
		}
	...
	public String excelCreator(HttpServletResponse response,File file) throws Exception {
		String msg=excelService.writeDataToCsv(response,file);
		return msg;
	}

	@Override
	public String pdfCreator(HttpServletResponse response,File file) throws Exception {
		String msg=pdfService.pdfGenerator(response,file);
		return msg;
	}
	
	@Override
	public String sentMail(String choice,File file) {
		String msg=mailService.sendMail(choice,file);
		return msg;
	}
